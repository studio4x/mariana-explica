...
export function CourseLessonMaterialsPanel() {
  ...
  const [draft, setDraft] = useState({
    ...
    storage_bucket: "",
    storage_path: "",
    storage_provider: null as ModuleAssetSummary["storage_provider"],
    external_url: "",
  ...
  const handleCreate = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await createAsset.mutateAsync({
        ...
        storage_provider: draft.source === "storage" ? draft.storage_provider ?? "r2" : null,
  ...
  const handleSaveAsset = async (assetId: string) => {
    ...
    try {
      await updateAsset.mutateAsync({
        ...
        storage_provider: editingAsset.source === "storage" ? editingAsset.storage_provider ?? "r2" : null,
