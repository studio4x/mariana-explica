  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleCreate} className="rounded-[1.75rem] border bg-white p-6 shadow-sm">
        <h2 className="font-display text-2xl font-bold text-slate-950">Novo produto</h2>
                  ...
                  <tr key={product.id} className="border-b last:border-b-0">
                    ...
                    <td className="py-4 pr-4">
                      <div className="flex flex-wrap gap-2">
                        <Button variant="outline" className="rounded-full" onClick={() => void publishProduct.mutateAsync(product.id)} disabled={publishProduct.isPending || product.status === "published"}>
                          Publicar
                        </Button>
                        <Button variant="outline" className="rounded-full" onClick={() => void archiveProduct.mutateAsync(product.id)} disabled={archiveProduct.isPending || product.status === "archived"}>
                          Arquivar
                        </Button>
                      </div>
                    </td>
