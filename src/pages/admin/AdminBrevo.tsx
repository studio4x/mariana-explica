...
import { CheckCircle2, KeyRound, Loader2, Mail, RefreshCw, Send, Users } from "lucide-react"
import { PageHeader, StatusBadge } from "@/components/common"
import { ErrorState } from "@/components/feedback"
import { Button } from "@/components/ui"
import { useAdminBrevoCatalog, useAdminBrevoContacts, useAdminBrevoHistory, useAdminBrevoMutation, useAdminBrevoOverview } from "@/hooks/useAdmin"
...
export function AdminBrevo() {
  const [tab, setTab] = useState<Tab>("config")
  const overview = useAdminBrevoOverview()
  const catalog = useAdminBrevoCatalog(tab === "config")
  ...
  const history = useAdminBrevoHistory({ query: historyQuery || undefined, status: historyStatus || undefined, offset: historyPage * PAGE_SIZE, limit: PAGE_SIZE })
  const contacts = useAdminBrevoContacts({ query: contactsQuery || undefined, status: contactsStatus || undefined, offset: contactsPage * PAGE_SIZE, limit: PAGE_SIZE })
  const mutations = useAdminBrevoMutation()
  const [apiKey, setApiKey] = useState("")
  const [testEmail, setTestEmail] = useState("")
  const [form, setForm] = useState(EMPTY_FORM)
  const [feedback, setFeedback] = useState<string | null>(null)
  const hydratedSettingsKeyRef = useRef<string | null>(null)
  const savedForm = settingsToForm(overview.data?.settings)
  const savedFormKey = JSON.stringify(savedForm)

  useEffect(() => {
    if (hydratedSettingsKeyRef.current === savedFormKey) return
    hydratedSettingsKeyRef.current = savedFormKey
    // The form is hydrated only when the authoritative backend snapshot changes.
    // This avoids wiping pending edits during ordinary query refetches.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setForm(savedForm)
  }, [savedForm, savedFormKey])

  if (overview.isLoading) return <div className="space-y-4"><PageHeader title="Integracao Brevo" description="Configuracao segura do transporte de e-mail e sincronizacao de leads." /><div className="h-32 animate-pulse rounded-3xl bg-slate-100" /></div>
  if (overview.isError) return <ErrorState title="Nao foi possivel carregar a integracao Brevo" message={overview.error instanceof Error ? overview.error.message : "Tente novamente."} onRetry={() => void overview.refetch()} />

  const credentials = overview.data?.credentials
  const settings = overview.data?.settings
  const lists = catalog.data?.lists ?? []
  ...
  const pageControls = (page: number, count: number, setPage: (value: number | ((value: number) => number)) => void) => <div className="mt-4 flex items-center justify-between text-sm text-slate-500"><span>{count} registro(s)</span><div className="flex gap-2"><Button size="sm" variant="outline" disabled={page === 0} onClick={() => setPage((value) => value - 1)}>Anterior</Button><Button size="sm" variant="outline" disabled={(page + 1) * PAGE_SIZE >= count} onClick={() => setPage((value) => value + 1)}>Proxima</Button></div></div>

  return <div className="space-y-6">
    <PageHeader title="Integracao Brevo" description="A Brevo e o transporte oficial de e-mails e o destino dos leads com consentimento explicito." actions={<div className="flex flex-wrap gap-2"><Button variant="outline" className="rounded-full" onClick={() => void overview.refetch()}><RefreshCw className="mr-2 h-4 w-4" />Atualizar</Button><Button variant="outline" className="rounded-full" onClick={() => void catalog.refetch()} disabled={catalog.isFetching}><RefreshCw className={`mr-2 h-4 w-4 ${catalog.isFetching ? "animate-spin" : ""}`} />Recarregar listas</Button></div>} />
    <div className="grid gap-4 md:grid-cols-3"><div className="rounded-3xl border bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><span className="text-sm text-slate-500">Integracao</span><StatusBadge label={settings?.enabled ? "Ativa" : "Desativada"} tone={settings?.enabled ? "success" : "neutral"} /></div><p className="mt-3 text-xl font-bold">{credentials?.configured ? "Credencial configurada" : "Credencial ausente"}</p></div><div className="rounded-3xl border bg-white p-5 shadow-sm"><span className="text-sm text-slate-500">Contatos</span><p className="mt-3 text-xl font-bold">{contacts.data?.count ?? 0}</p><p className="text-xs text-slate-500">fila local e estado confirmado</p></div><div className="rounded-3xl border bg-white p-5 shadow-sm"><span className="text-sm text-slate-500">Historico</span><p className="mt-3 text-xl font-bold">{history.data?.count ?? 0}</p><p className="text-xs text-slate-500">registros locais e eventos Brevo</p></div></div>
    <div className="flex flex-wrap gap-2 rounded-2xl bg-slate-100 p-2"><Button variant={tab === "config" ? "default" : "ghost"} onClick={() => setTab("config")}><KeyRound className="mr-2 h-4 w-4" />Configuracao</Button><Button variant={tab === "history" ? "default" : "ghost"} onClick={() => setTab("history")}><Mail className="mr-2 h-4 w-4" />Historico</Button><Button variant={tab === "contacts" ? "default" : "ghost"} onClick={() => setTab("contacts")}><Users className="mr-2 h-4 w-4" />Contatos</Button></div>
    {feedback ? <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900">{feedback}</div> : null}
    {tab === "config" && hasUnsavedSettings ? <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">Existem alteracoes nao salvas nesta configuracao. Clique em salvar definicoes para manter a lista selecionada apos recarregar a pagina.</div> : null}
    ...
    {tab === "contacts" ? <section className="rounded-3xl border bg-white p-6 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-bold">Contatos sincronizados</h2><p className="text-sm text-slate-500">Estado local, consentimento e confirmacao retornada pela Brevo.</p></div><div className="flex gap-2"><Button variant="outline" onClick={() => void mutations.retryFailed.mutateAsync()}><RefreshCw className="mr-2 h-4 w-4" />Retentar falhados</Button><Button variant="outline" onClick={() => void contacts.refetch()}>Atualizar</Button></div></div><div className="mt-5 flex flex-wrap gap-3"><input value={contactsQuery} onChange={(e) => { setContactsQuery(e.target.value); setContactsPage(0) }} placeholder="Buscar por e-mail ou ID Brevo" className="h-10 min-w-[260px] flex-1 rounded-xl border px-3 text-sm" /><select value={contactsStatus} onChange={(e) => { setContactsStatus(e.target.value); setContactsPage(0) }} className="h-10 rounded-xl border px-3 text-sm"><option value="">Todos os estados</option><option value="queued">queued</option><option value="processing">processing</option><option value="synced">synced</option><option value="failed">failed</option></select></div>{contacts.isLoading ? <div className="py-8 text-sm text-slate-500">A carregar contatos...</div> : contacts.isError ? <ErrorState title="Falha nos contatos" message={contacts.error instanceof Error ? contacts.error.message : "Tente novamente."} onRetry={() => void contacts.refetch()} /> : <div className="mt-5 overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr className="border-b text-xs uppercase text-slate-500"><th className="p-3">Contato</th><th className="p-3">Consentimento</th><th className="p-3">Brevo</th><th className="p-3">Sincronizacao</th><th className="p-3" /></tr></thead><tbody>{(contacts.data?.rows ?? []).map((row) => <tr key={row.id} className="border-b last:border-0"><td className="p-3">{row.email}<div className="text-xs text-slate-400">{row.consent_source}</div></td><td className="p-3">{formatDateTime(row.consent_at)}</td><td className="p-3"><div>{row.brevo_contact_id ?? "-"}</div><div className="text-xs text-slate-400">lista {row.list_id ?? "-"}</div></td><td className="p-3"><StatusBadge label={row.status} tone={row.status === "failed" ? "danger" : row.status === "synced" ? "success" : "neutral"} />{row.last_error ? <div className="mt-1 max-w-xs text-xs text-red-600">{row.last_error}</div> : null}</td><td className="p-3 text-right"><Button size="sm" variant="outline" onClick={() => void mutations.syncContact.mutateAsync(row.id)} disabled={mutations.syncContact.isPending}>Sincronizar</Button></td></tr>)}</tbody></table>{(contacts.data?.rows ?? []).length === 0 ? <p className="py-8 text-center text-sm text-slate-500">Nenhum lead com consentimento foi enfileirado.</p> : null}{pageControls(contactsPage, contacts.data?.count ?? 0, setContactsPage)}</div>}</section> : null}
  </div>
}
