...
export function AdminSessionRecovery() {
  ...
  const handleRefresh = async () => {
    ...
    setErrorMessage("Não foi possível recuperar a sessão agora. Tenta novamente em instantes.")
  }
  ...
  return (
    <div className="flex min-h-[60vh] items-center justify-center px-6">
      <div className="w-full max-w-xl rounded-[2rem] border bg-white p-8 text-center shadow-sm">
        ...
        <h1 className="mt-5 text-2xl font-bold text-slate-950">Sessão do admin precisa ser recarregada</h1>
        <p className="mt-3 text-sm leading-7 text-slate-600">
          A validação do perfil falhou, mas a tua sessão pode ainda estar válida. Recarrega a sessão para tentar
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-center">
          <Button onClick={() => void handleRefresh()} disabled={isRefreshing} className="rounded-full">
            ...
            {isRefreshing ? "A recarregar..." : "Recarregar sessão"}
          </Button>
          <Button
            variant="outline"
