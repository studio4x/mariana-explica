...
import { renderPlatformEmailLayout, type EmailContent, type EmailLayoutInput } from "./email-layout.ts"

export { renderPlatformEmailLayout } from "./email-layout.ts"
export type { EmailContent, EmailLayoutInput } from "./email-layout.ts"
...
async function buildEmailContentFromPlatformTemplate(
  ...
  const email = renderPlatformEmailLayout({
    headerLogoUrl,
...
export async function buildPlatformEmailLayout(
  client: SupabaseClient,
  input: Omit<EmailLayoutInput, "headerLogoUrl">,
): Promise<EmailContent> {
  const headerLogoUrl = await fetchPlatformBrandingLogoUrl(client)
  return renderPlatformEmailLayout({ ...input, headerLogoUrl, appBaseUrl: getAppBaseUrl() })
}
